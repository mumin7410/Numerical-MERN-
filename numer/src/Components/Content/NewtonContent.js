...
const Newton = ({MethodTopicText,MethodDescripeText,Memes}) => {
      ...
      const BisectionCal = () => {
        ...
        //calculater
        while(true) {
          Fa = math.parse(EQ).evaluate({x: A}); //F(x)
          Fdiff_a = math.derivative(EQ, 'x').evaluate({x: A}) //F'(x)
          X = A - (Fa / Fdiff_a);
          Err = math.parse(ErrEquation).evaluate({xnew:X, xold:A});
          
          F_answer = math.parse(EQ).evaluate({x: X});

          data.push({calories:"Iteration "+i, fat:X, carbs:math.abs(Err),Ct:parseFloat(A).toFixed(4),F_answer:F_answer});
          A = X;
